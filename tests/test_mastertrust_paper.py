"""Comprehensive smoke test for paper-trading mode against MasterTrust."""
...
spec = importlib.util.spec_from_file_location(
    "fenix.mastertrust",
    fenix_path / "fenix" / "mastertrust.py"
)
mastertrust = importlib.util.module_from_spec(spec)
sys.modules["fenix.mastertrust"] = mastertrust
spec.loader.exec_module(mastertrust)

MasterTrust = mastertrust.MasterTrust
...
def test_paper_mode():
    """Run 14 comprehensive paper-trading scenarios against MasterTrust."""

    config = {"paper_mode": True}
    broker = MasterTrust(config)
    broker.authenticate()
    ...
    print("\n" + "="*70)
    print("PAPER-TRADING SMOKE TEST - MasterTrust (14 scenarios)")
    print("="*70 + "\n")
    ...
    # Test 13: Square-off — MasterTrust does not expose square_off_position;
    ...
    try:
        ...
        config_live = {}
        broker_live = MasterTrust(config_live)
        try:
