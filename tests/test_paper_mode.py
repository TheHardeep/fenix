"""Comprehensive smoke test for paper-trading mode across all brokers."""
...
spec = importlib.util.spec_from_file_location(
    "fenix.aliceblue",
    fenix_path / "fenix" / "aliceblue.py"
)
aliceblue = importlib.util.module_from_spec(spec)
sys.modules["fenix.aliceblue"] = aliceblue
spec.loader.exec_module(aliceblue)
...
def test_paper_mode():
    """Run 14 comprehensive paper-trading scenarios."""

    config = {"paper_mode": True}
    broker = AliceBlue(config)
    broker.authenticate()
    ...
    print("\n" + "="*70)
    print("PAPER-TRADING SMOKE TEST (14 scenarios)")
    print("="*70 + "\n")
    ...
    try:
        ...
        # Tick at 515 should fill the limit (LTP <= 520)
    ...
    try:
        ...
        # Tick at 523 triggers and fills (trigger 522 crossed, fills at market/limit)
    ...
    try:
        ...
        try:
            broker.place_order(
                ...
                side="SELL",
                price=510.0,
                trigger=525.0,  # Above current LTP of 523
                product="MIS",
                validity="DAY",
                variety="REGULAR",
                unique_id="test_6"
    ...
    try:
        ...
        # Tick to trigger Test 5's SL and add a new position
        ...
        pos = positions[0]
        # Just verify position has accumulated fills
    ...
    try:
        ...
        assert len(positions) == 1
        pos = positions[0]
        # MTM = net_qty * (ltp - avg_price) if net > 0
        # Just verify MTM is positive (long position, price went up)
    ...
    # Test 13: Square-off places opposing SELL market order, flattens position
    try:
        print("[13] Square-off flattens position...")
        ...
        # Tick to fill the square-off sell
        broker.on_tick(token=12345, ltp=600.0)
    ...
    try:
        ...
        config_live = {}  # paper_mode=False
        broker_live = AliceBlue(config_live)
        try:
