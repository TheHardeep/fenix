# Motilal Oswal — Live Feed

Motilal Oswal (MOXTS) live market-data adapter for the Fenix-Pro feed interface.

| | |
|---|---|
| **Class** | `fenixpro.motilaloswal` |
| **`id`** | `motilaloswal` |
| **Module** | `fenixpro/motilaloswal.py` |
| **Extends** | [`fenixpro.iifl`](#/pro/iifl) |
| **Transport** | `socketio` · JSON events (XTS) |
| **Base URL** | `https://moxtsapi.motilaloswal.com:3000` |
| **Auth params** | `api_key`, `api_secret` |
| **Feed types** | `LTP`, `DEPTH` |

Motilal Oswal exposes the XTS market-data API on its own infrastructure, so the Fenix-Pro
...
Identical to IIFL — pass `params` with the MOXTS market-data API key/secret, and the parent
class handles login, header building, and Socket.IO connect.

```python
from fenixpro import motilaloswal, FeedType
...
| `fno_tokens` | Optional | Output of `MotilalOswal.load_fno_tokens()` — enables `subscribe_fno_token`. |
