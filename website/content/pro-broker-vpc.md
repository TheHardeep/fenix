...
VPC live market-data adapter for the Fenix-Pro feed interface.

| | |
|---|---|
| **Class** | `fenixpro.vpc` |
| **`id`** | `vpc` |
| **Module** | `fenixpro/vpc.py` |
| **Extends** | [`fenixpro.iifl`](#/pro/iifl) |
| **Transport** | `socketio` · JSON events (XTS) |
| **Base URL** | `http://122.160.19.15:3000` |
| **Auth params** | `api_key`, `api_secret` |
| **Feed types** | `LTP`, `DEPTH` |

VPC runs the XTS market-data API on its own infrastructure, so the Fenix-Pro adapter is a thin
subclass of [`iifl`](#/pro/iifl): it inherits the full Socket.IO connect, subscribe, and
parser flow and overrides only the `base` URL.

## Construction

Identical to IIFL — pass `params` with the VPC market-data API key/secret, and the parent
class handles login, header building, and Socket.IO connect.

```python
from fenixpro import vpc, FeedType
...
| `fno_tokens` | Optional | Output of `VPC.load_fno_tokens()` — enables `subscribe_fno_token`. |
