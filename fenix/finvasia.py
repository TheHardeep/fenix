...
class finvasia(Broker):
    ...
    def create_eq_tokens(cls) -> dict:
        ...
        df_bse = cls.data_reader(
            cls.base_urls["market_data"].replace(ExchangeCode.NFO, ExchangeCode.BSE),
            filetype="csv",
        )
        ...
        df_nse = cls.data_reader(
            cls.base_urls["market_data"].replace(ExchangeCode.NFO, ExchangeCode.NSE),
            filetype="csv",
        )
        df_nse = df_nse[df_nse["Instrument"] == "EQ"][
            ["Symbol", "TradingSymbol", "TickSize", "Token", "LotSize", "Exchange"]
        ]
        df_nse.rename(
            {"Symbol": "Index", "TradingSymbol": "Symbol", "token": "Token"},
            axis=1,
            inplace=True,
        )
    ...
    def create_indices(cls) -> dict:
        ...
        """
        df = cls.data_reader(
            cls.base_urls["market_data"].replace("NFO", "NSE"), filetype="csv"
        )
    ...
    def create_fno_tokens(cls):
        ...
        Stores them in the finvasia.fno_tokens Dictionary.
        ...
        try:
            ...
            df_bfo = df_bfo[
                ((df_bfo["Symbol"] == "BSXOPT") | (df_bfo["Symbol"] == "BKXOPT"))
                & ((df_bfo["Instrument"] == "OPTIDX"))
            ]

            df_bfo["Symbol"] = df_bfo["Symbol"].replace(
            ...
            df.rename(
                {"OptionType": "Option", "Symbol": "Root", "TradingSymbol": "Symbol"},
                axis=1,
                inplace=True,
            )
    ...
    def modify_order(
        ...
        jdata = {
            ...
            "ret": (
                cls.req_validity.get(validity, order_info["ret"])
                if validity
                else order_info["ret"]
            ),
            "uid": headers["uid"],
    ...
    def fetch_holdings(
        ...
        for product in cls.resp_product:
            jdata = {"uid": headers["uid"], "actid": headers["uid"], "prd": product}
